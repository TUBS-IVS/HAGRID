...
New-Item -ItemType Directory -Force "$tmp\hagrid\simulation\target", "$tmp\hagrid\simulation\input", "$tmp\external", "$tmp\runs\lausitz" | Out-Null
# Repo-Wurzel: pom.xml + external\ (Tiefenpruefung); die Wurzel-POM nennt das Modul, das Modul seine artifactId
Set-Content "$tmp\pom.xml" '<project><modules><module>hagrid/simulation</module></modules></project>'
Set-Content "$tmp\hagrid\simulation\pom.xml" '<project><artifactId>hagrid</artifactId></project>'
Set-Content "$tmp\hagrid\simulation\input\README.md" 'marker'
...
New-Jar "$tmp\hagrid\simulation\target\hagrid-1.0-SNAPSHOT.jar" @($runner) $null
New-Jar "$tmp\hagrid\simulation\target\ok.jar"                  @($runner) 'hagrid.core.simulation.HAGRIDSimulationRunner'
New-Jar "$tmp\hagrid\simulation\target\badmanifest.jar"         @($runner) 'hagrid.core.simulation.NichtImJar'

$good = "@echo off`r`ncd /d `"%~dp0..\..\hagrid\simulation`"`r`nset `"JAR=target\hagrid-1.0-SNAPSHOT.jar`"`r`njava -Dhagrid.pipeline.root=. -cp `"%JAR%`" hagrid.core.simulation.HAGRIDSimulationRunner concept=x`r`n"
...
Write-Script "$tmp\runs\lausitz\wrap.bat" "@echo off`r`ncd /d `"%~dp0..\..\hagrid\simulation`"`r`ncall run_missing_chain.bat`r`n"
$out = & $check -RepoRoot $tmp -Scripts "$tmp\runs"
...
Write-Script "$tmp\runs\lausitz\jarok.bat" "@echo off`r`ncd /d `"%~dp0..\..\hagrid\simulation`"`r`nset `"JAR=target\ok.jar`"`r`njava -Dhagrid.pipeline.root=. -jar `"%JAR%`" concept=x`r`n"
...
Write-Script "$tmp\runs\lausitz\jarbad.bat" "@echo off`r`ncd /d `"%~dp0..\..\hagrid\simulation`"`r`nset `"JAR=target\badmanifest.jar`"`r`njava -Dhagrid.pipeline.root=. -jar `"%JAR%`" concept=x`r`n"
$out = & $check -RepoRoot $tmp -Scripts "$tmp\runs"
...
Write-Host 'Fall 9: -pl :hagrid (artifactId-Selektor) ist gueltig'
