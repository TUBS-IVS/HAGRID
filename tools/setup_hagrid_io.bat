...
REM =========================================================================
REM Creates the new input/ directory structure and copies/links files
...
echo [1/7] Creating input directories...
mkdir "%PIPELINE%\input\hannover\config" 2>nul
mkdir "%PIPELINE%\input\hannover\demand" 2>nul
mkdir "%PIPELINE%\input\hannover\geodata" 2>nul
mkdir "%PIPELINE%\input\hannover\hubs" 2>nul
mkdir "%PIPELINE%\input\hannover\network" 2>nul
mkdir "%PIPELINE%\input\hannover\vehicles" 2>nul
...
if exist "%PIPELINE%\input\config.xml" (
    copy "%PIPELINE%\input\config.xml" "%PIPELINE%\input\hannover\config\config.xml" >nul
    echo     config.xml copied
)
if exist "%PIPELINE%\sim-input\sim-config.xml" (
    copy "%PIPELINE%\sim-input\sim-config.xml" "%PIPELINE%\input\hannover\config\sim-config.xml" >nul
    echo     sim-config.xml copied
)
if exist "%PIPELINE%\sim-input\algorithm.xml" (
    copy "%PIPELINE%\sim-input\algorithm.xml" "%PIPELINE%\input\hannover\config\jsprit-algorithm.xml" >nul
...
if exist "%PIPELINE%\input\HAGRID_vehicleTypes2.0.xml" (
    copy "%PIPELINE%\input\HAGRID_vehicleTypes2.0.xml" "%PIPELINE%\input\hannover\vehicles\" >nul
...
if exist "%PIPELINE%\input\hubs\KEP-hubs_v3.csv" (
    copy "%PIPELINE%\input\hubs\KEP-hubs_v3.csv" "%PIPELINE%\input\hannover\hubs\" >nul
    echo     KEP-hubs_v3.csv copied
)
if exist "%PIPELINE%\input\hubs\standorte_von_dhl.de.csv" (
    copy "%PIPELINE%\input\hubs\standorte_von_dhl.de.csv" "%PIPELINE%\input\hannover\hubs\" >nul
    echo     standorte_von_dhl.de.csv copied
)
if exist "%PIPELINE%\input\hubs\standorte_von_paket.net" (
    xcopy "%PIPELINE%\input\hubs\standorte_von_paket.net" "%PIPELINE%\input\hannover\hubs\standorte_von_paket.net\" /E /I /Q >nul
    echo     standorte_von_paket.net/ copied
)
if exist "%PIPELINE%\input\geodata" (
    xcopy "%PIPELINE%\input\geodata" "%PIPELINE%\input\hannover\geodata\" /E /I /Q >nul
    echo     geodata/ copied
)
...
if exist "%PIPELINE%\input\demand" (
    xcopy "%PIPELINE%\input\demand" "%PIPELINE%\input\hannover\demand\" /E /I /Q >nul
...
echo.
echo   New structure:
echo     %PIPELINE%\input\       All pipeline inputs
...
echo     input\vehicles\  (base template, read by pipeline)
