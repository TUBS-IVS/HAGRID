...
public class DeliveryGenerator implements Runnable {
                                                ...
                                                entry -> { // Transform the value
                                                        ...
                                                        return entry.getValue().stream() // Stream the SimpleFeature
                                                                        ...
                                                                        .map(simpleFeature -> createDelivery(
                                                                                        simpleFeature, provider)) // Convert
                                                                                                                  // each
                                                                                                                  // SimpleFeature
                                                                                                                  // to
                                                                                                                  // Delivery
                ...
                if (hagridConfig.isWhiteLabel()) {
                        ...
                        return Optional.ofNullable((String) feature.getAttribute(nameInShape))
                                        .orElseThrow(() -> new IllegalArgumentException(
                                                        "No attribute found for provider: " + provider + " and "
                                                                        + nameInShape));
                }
        }

        private void addParcelLockerServices(
                        Map<String, ArrayList<Delivery>> deliveries, Map<Id<Hub>, Hub> parcelLockerList) {

                LOGGER.info(deliveries.keySet().toString());
                ...
                parcelLockerList.values().stream()
                                ...
                                .forEach(hub -> {
                                        ...
                                        String deliveryKey = getDeliveryKey(plz, hagridConfig.isWhiteLabel());

                                        

                                        ArrayList<Delivery> deliveryList = deliveries.computeIfAbsent(deliveryKey,
                                                        k -> new ArrayList<>());
                                        List<String> possibleDeliveryKeys = new ArrayList<>();

                                        if (deliveryList.isEmpty()) {
                                                possibleDeliveryKeys = findPossibleDeliveryKeys(deliveries, plz,
                                                                hagridConfig.isWhiteLabel());
                                        }
                                        LOGGER.info("Found possible keys: " + possibleDeliveryKeys + " for plz: " + plz
                                                        + " and Key: " + deliveryKey);

                                        // Delivery parcelLockerDelivery = createParcelLockerDelivery(hub, deliveryKey);

                                        // deliveries.get(deliveryKey).add(parcelLockerDelivery);

                                });
        }

        private static String getDeliveryKey(Integer plz, boolean isWhiteLabel) {
                return isWhiteLabel ? "wl_" + plz : "dhl_" + plz;
        }
        ...
        private static List<String> findPossibleDeliveryKeys(Map<String, ArrayList<Delivery>> deliveries, Integer plz,
                ...
                return deliveries.keySet().stream()
                                .filter(key -> key.startsWith(isWhiteLabel ? "wl_" + plz : "dhl_" + plz))
         ...
         * and each postal
         * code. It also calculates and logs the ratios of B2B deliveries and parcels to
         * the
