...
import hagrid.utils.routing.StateIds;
...
/**
	 ...
	 *
	 */
public class TimeWindowConstraintWithDriverTime implements HardActivityConstraint {

		private final RouteAndActivityStateGetter states;
		
		private final VehicleRoutingTransportCosts routingCosts;
		
		private final double maxDriverTime;
...
//			if vehicle cannot even manage direct-route - break
		...
		private double getLatestNewActStart(TourActivity newAct, TourActivity nextAct, JobInsertionContext iFacts, double latestVehicleArrival, double departureAtNewAct) {
			...
			double latestNewAct = latestNextActStart - routingCosts.getTransportTime(newAct.getLocation(), nextAct.getLocation(), 
		...
		private double getVehicleDeparture(JobInsertionContext iFacts, TourActivity newActivity) {
			double tpTime = routingCosts.getTransportTime(iFacts.getNewVehicle().getStartLocation(), newActivity.getLocation(), iFacts.getNewDepTime(), iFacts.getNewDriver(), iFacts.getNewVehicle());
