...
Gemessen am 21.09. auf dem Dev: Repo-Wurzel 51 Zeichen, längster vorhandener Pfad unter `hagrid/hagrid-matsim-output/` 202 Zeichen relativ zum Modul, zusammen **262 Zeichen, also heute schon über der klassischen Grenze**; nach dem Umbau 273. `LongPathsEnabled` ist auf dem Dev **nicht** gesetzt. Java (NIO) schreibt solche Pfade trotzdem, deshalb existiert die Datei. Für PowerShell 5.1, Python und Werkzeuge außerhalb von Java gilt das nicht automatisch, und `core.longpaths=true` betrifft nur Git.
...
6. **Rewrite-Gate, repo-weit** (§6.5): Über **alle getrackten Dateien** (`git grep -nIP … -- .`), vier Muster: (a) `hagrid[\\/](?!simulation[\\/]|demand[\\/])` für String-Pfade, (b) `Path\.(of|get)\(\s*"hagrid"\s*,` für strukturierte Java-Pfade, (c) `Join-Path\s+[^\r\n]*'hagrid'` für PowerShell, (d) `\\HAGRID\\hagrid\b` für Absolutpfade. Erwartung: leer bis auf eine **explizite Allowlist** mit Begründung je Eintrag (Migrations- und Prüfskripte, deren Selbsttests, `HagridPaths` mit dem neuen Literal, historische Specs/Pläne unter `docs/superpowers/`, `analysis/hannover/sweep/provenance/*.patch`, METHODS-LOG/BACKLOG-DONE als Historie). Jeder Treffer außerhalb der Allowlist ist ein Befund.
