...
6. **Rewrite-Gate, repo-weit** (§6.5): Über **alle getrackten Dateien** (`git grep -nIP … -- .`), fünf Muster: (a1) `hagrid[\\/](?!simulation\b|demand\b|core\b|hannover\b|lausitz\b|\{)` für String-Pfade (Wortgrenzen im Lookahead, damit `hagrid/simulation` am Tokenende kein Treffer ist), (a2) `[\\/]hagrid["']` für die von (a1) nicht gefangene End-of-Path-Form `..\hagrid"`, (b) `Path\.(of|get)\(\s*"hagrid"\s*,` für strukturierte Java-Pfade, (c) `Join-Path\s+[^\r\n]*'hagrid'` für PowerShell, (d) `\\HAGRID\\hagrid\b` für Absolutpfade. Erwartung: leer bis auf eine **explizite Allowlist** mit Begründung je Eintrag (Migrations- und Prüfskripte, deren Selbsttests, `HagridPaths` mit dem neuen Literal, historische Specs/Pläne unter `docs/superpowers/`, `analysis/hannover/sweep/provenance/*.patch`, METHODS-LOG/BACKLOG-DONE als Historie, zusätzlich `docs/legacy/` [eingefrorener Inhalt] und `.superpowers/` [getrackte historische SDD-Berichte]). Jeder Treffer außerhalb der Allowlist ist ein Befund.
...
Befunde aus Planung und Ausführung von Task 0–4 (Worktree `HAGRID-r3`, Commit `73a8324`), numeriert 1–8 (Planungsstand) und 9–16 (bei der Ausführung aufgetreten):
...
15. Gate-Muster a1 nutzt Wortgrenzen im Lookahead (`simulation\b|demand\b|…`), damit `hagrid/simulation` am Tokenende kein Treffer ist; die Allowlist enthält zusätzlich `docs/legacy/` (eingefrorener Inhalt) und `.superpowers/` (getrackte historische SDD-Berichte). Verbleibende a1-Treffer bei `e5cf0cb` sind Dachordner- bzw. Legacy-Referenzen und ein Test-DisplayName über das ALTE Layout — erklärt in `task-3-report.md`.
