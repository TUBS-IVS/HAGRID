...
- Ablauf je Maschine: `git pull` → `git submodule update` (Gitlink unverändert, also ohne Netzlast) → `tools/migrate-input-layout.ps1` (schiebt `parcel-demand-2-matsim-pipeline/hagrid-input/*` idempotent nach `hagrid/input/{common,hannover,lausitz}` und `hagrid-output/`, `hagrid-matsim-output/`, `routerCache/`, `logs/`, `target/` ins Modul) → `mvn -q clean install` → ein Fingerprint-Lauf wie §7.1 gegen den lokal notierten Wert.
- **`clean` ist Pflicht, nicht Geschmack**: das Migrationsskript zieht das alte `target/` mit ins Modul (es steht in seiner Phase-A-Liste). Ohne `clean` liegen darin noch die Klassen des alten Paketlayouts, und `shade` packt beide Layouts in dasselbe Jar — der Lauf startet dann je nach Classpath-Reihenfolge alten Code. Geändert wurde bewusst das Rezept, nicht das Skript: das alte `target/` soll mitziehen (sonst bliebe es im alten Modulordner zurück und das Skript könnte ihn nicht entfernen), es darf nur nicht weiterverwendet werden.
...
Nebenbefunde: **fünf** versehentlich getrackte GC-Logs (`logs/jvm_gc.log.0` bis `.log.4`, `*.log` deckt `.log.0` nicht) und eine getrackte `.bak_before_abort`-Kopie eines Skripts; `.0/.1` fielen in Schritt 1, die drei großen (`.2`–`.4`, je 21 MB) erst in der abschließenden Fix-Welle auf — beim Zählen in Schritt 1 wurden sie übersehen. Dateien bleiben jeweils auf der Platte und werden von der `hagrid/logs`-Regel abgedeckt; der Provenance-Patch des Hannover-Sweeps enthält den alten Modulnamen und steht auf der Allowlist von Gate 6.
