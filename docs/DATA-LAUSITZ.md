...
**git-ignored** (covered by the `hagrid/simulation/input/**` rule in
`.gitignore`) and must be reproduced locally by running the copy commands below.

## Staged Files

| Destination (under `hagrid/simulation/input/lausitz/`) | Source |
|---|---|
...
```bash
HI="hagrid/simulation/input/lausitz"
...
- The `lausitzBaseConfig()` getter in `HagridPaths.java` resolves
  `hagrid/simulation/input/lausitz/config/lausitz-v2024.2-100pct.config.xml` under the study-area-scoped
  `inputBase`.
- All staged files are large binaries and must never be committed to the repository.
- The `.gitignore` rule `hagrid/simulation/input/**` covers all contents of
